eimport { ciclosData, farmaciasCoords, obtenerCicloActual, formatearFechaTurno, limpiarTelefono } from './data.js';
...
const todasOrdenadas = todas.sort((a, b) => a.nombre.localeCompare(b.nombre));
  ...
  todasOrdenadas.forEach((f, i) => {
    ...
    div.onclick = () => {
      ...
      if (esEscritorio) {
        if (mapDesktop && markersDesktop && markersDesktop[i]) {
          markersDesktop[i].openPopup();
          if (coords) mapDesktop.setView(coords, 16);
        }
        setTimeout(() => {
          const mapElement = document.getElementById('map-desktop');
          if (mapElement) {
